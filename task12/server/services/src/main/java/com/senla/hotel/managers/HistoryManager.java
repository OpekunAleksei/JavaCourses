...
public class HistoryManager implements IHistoryManager {
    ...
    public void settleInRoom(Guest guest, Room room) throws SQLException {
        ...
        try {
            ...
            historyDao.create(HibernateUtil.getIstance().getSession(), historyDao.getMiracleHistory(guest, room));
    ...
    public void evictedFromRoom(Guest guest, Room room) throws SQLException {
        ...
        try {
            ...
            historyDao.update(HibernateUtil.getIstance().getSession(), historyDao.getMiracleHistory(guest, room));
            DbConnector.getIstance().getConnection().commit();
        } catch (SQLException ex) {
            logger.error(new Date() + " " + ex.getMessage());
    ...
    public List<Guest> getListLeftGuestThisRoom(Room room, Integer count) throws SQLException {
        ...
        for (int i = 0; i < listId.size(); i++) {
            leftGuests.add(guestDao.getById(HibernateUtil.getIstance().getSession(), listId.get(i)));
        }
        return leftGuests;
    ...
    public List<Room> getListOfRoomsAvailableByDate(Date date, List<Room> list) throws SQLException {
        ...
        for (int i = 0; i < listId.size(); i++) {
            avaliableRooms.add(roomDao.getById(HibernateUtil.getIstance().getSession(), listId.get(i)));
        }
    ...
    public List<Service> getGuestServices(Guest guest, Room room, String sort) throws SQLException {
        ...
        try {
            ...
            list = historyDao.getServices(session, guest, room, sort);
        ...
        // return serviceDao.getById(HibernateUtil.getIstance().getSession(), historyDao.getIdSortingServices(DbConnector.getIstance().getConnection(), room, guest, sort));
