...
public class Configuration {
    ...
    public Configuration() {
        ...
        try (InputStream is = ClassLoader.getSystemClassLoader().getResourceAsStream("com/senla/hotel/configuration/injectProperties.properties")) {
            injectProperties.load(is);
        } catch (IOException ex) {
            logger.error(new Date() + " " + ex.getMessage());
        }
        try (InputStream is = ClassLoader.getSystemClassLoader().getResourceAsStream("com/senla/hotel/configuration/properties.properties")) {
            properties.load(is);
        } catch (IOException ex) {
            logger.error(new Date() + " " + ex.getMessage());
        }
