...
public class GetSortServisecOfGuestByCategory implements IAction {
    ...
    public void execute() {
        ...
        TextWorker.getInstance().println(Request.getInstance().pull(TextWorker.getInstance().createLine("getListServiceOfGuest", TextWorker.getInstance().createData(id.toString(), number.toString(), SortName.category.toString()))));
    }
}
