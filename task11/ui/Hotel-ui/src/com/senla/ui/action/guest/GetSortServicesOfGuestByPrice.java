...
public class GetSortServicesOfGuestByPrice implements IAction {
   ...
   getListOfGuests.execute();
        ...
        TextWorker.getInstance().println(Request.getInstance().pull(TextWorker.getInstance().createLine("getSortListServiceGuestByPrice", TextWorker.getInstance().createData(id.toString(), number.toString()))));
